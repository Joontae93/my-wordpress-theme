...
class Asset_Loader {
	 ...
	 * @var $asset_file
	 */
	private string|false $asset_file;
	...
	public function __construct( string $id, Asset_Load_Type $load_type, string $folder = '', array $deps = array(), $js_load_strategy = array( 'strategy' => 'defer' ) ) {
		...
		if ( $this->asset_file ) {
			$this->deps    = array_unique( array_merge( $this->asset_file['dependencies'], array( 'global' ) ) );
	 ...
	 */
	private function get_asset_file(): string|false {
		$file = get_stylesheet_directory() . $this->location . '.asset.php';
		if ( file_exists( $file ) ) {
			$file_contents = file_get_contents( $file ); // phpcs:ignore WordPress.WP.AlternativeFunctions
	...
	private function enqueue_page_style() {
		...
		$version = $this->version ?? filemtime( get_stylesheet_directory() . $this->location . '.css' );
		wp_enqueue_style(
			$this->id,
			$src,
			$this->deps,
